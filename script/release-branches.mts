...
/* v8 ignore start -- @preserve: GitRunner injection unit-tests orchestration without mutating git */
async function runGit(
  args: string[],
  options: { allowFailure?: boolean } = {},
): Promise<GitResult> {
  try {
    const result = await execFileAsync('git', args, { cwd: rootDir, encoding: 'utf8' })
...
export class ReleaseBranchManager {
  ...
  private async requireExactRemote(branch: string) {
    ...
    if (local.stdout.trim() !== remote.stdout.trim()) {
      throw new Error(`Local ${branch} is not synchronized with origin/${branch}`)
    }
  }

  private async switchToRemoteBranch(branch: string) {
    await this.requireRemoteBranch(branch)
    if (await this.branchExists(`refs/heads/${branch}`)) {
  ...
  async prepareDevelop() {
    ...
    if (await this.branchExists('refs/heads/develop')) {
      throw new Error('Local develop exists without origin/develop; inspect it first')
    }
  ...
  async promote(action: 'preview' | 'stable') {
    ...
    if (!(await this.branchExists(`refs/remotes/origin/${target}`))) {
      if (action !== 'preview') throw new Error(`Remote branch origin/${target} does not exist`)
      if (await this.branchExists(`refs/heads/${target}`)) {
        throw new Error(`Local ${target} exists without origin/${target}; inspect it first`)
      }
...
/* v8 ignore start -- @preserve: the CLI delegates to the tested parser and manager */
if (isCli) {
  try {
