...
<template>
  <NThing
    class="relative w-full overflow-hidden bg-(--van-background-2) text-nowrap *:w-full"
  ...
  >
    ...
    <template #header>
      <div class="mt-2 -mb-2 flex items-center">
        ...
        <div
          class="mx-1 flex items-center text-(--nui-primary-color)"
          v-else-if="customUser.gender == 'Female'"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
        ...
        <span class="mr-1 text-xs font-normal text-(--nui-primary-color)"
          >Lv{{ user.customUser.expInfo.level }}</span
        >
      </div>
    </template>
    <template #description>
      <div class="flex w-full flex-wrap gap-1 py-1">
        <VanTag type="primary">
      ...
      <div class="flex w-[60%]! items-center">
        ...
        <NProgress
          ...
          status="info"
          :percentage="
            ((user.customUser.expInfo.$exp ?? 0) / (user.customUser.expInfo.$nextLevelExp ?? 0)) *
            100
          "
    ...
    <div v-if="!isSmall && !isEmpty(user.customUser.expInfo.$badges)" class="w-full">
      <VanTag type="primary" plain size="large" class="mb-2 ml-3">
        <NIcon size="1rem">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            xmlns:xlink="http://www.w3.org/1999/xlink"
      ...
      <div class="relative flex h-full w-full flex-nowrap items-center justify-around px-1">
        <div
