...
export class Fork {
  ...
  public async getForks() {
    ...
    const fetch = this.sdk.requester.create({ responseType: 'text', baseURL: undefined })
    let result = ''
    ...
    await Promise.all(
      source.map(async baseURL => {
        try {
          ...
          acs.push(ac)
          const body = await fetch<string>(path, { baseURL, signal: ac.signal })
          result = body
   ...
   */
  public async autoPickFork(forks: string[]) {
      ...
      await Promise.all(
        forks.map(async fork => {
          try {
            ...
            await fetch(path, { baseURL: fork, signal: abortController.signal })
            clearTimeout(stopTimeout)
            const end = Date.now()
            const time = end - begin
            record.push([fork, time])
            console.debug(`test url ${fork} connected time ${time}ms`)
            abortController.abort()
          } catch {
            record.push([fork, false])
            console.debug(`test url ${fork} can not connected`)
          }
        })
      )
    } catch (err) {
      console.debug('test aborted', err)
    }
    const result = sortBy(
      record.filter(v => v[1] != false),
      v => v[1]
    )[0]
    console.debug(`test done`, result)
